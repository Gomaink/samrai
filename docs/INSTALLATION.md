...
Open `http://127.0.0.1:24600/`. The Compose file pulls `ghcr.io/gomaink/samrai:0.2.0-rc.9`, maps host port `24600` to container port `8080`, and stores persistent state in the mounted `/data` volume. The container runs as a non-root user with a read-only root filesystem.
...
Copy-Item "C:\samrai\samrai-v0.2.0-rc.8\data\*" "C:\samrai\samrai-v0.2.0-rc.9\data" -Recurse -Force
Copy-Item "C:\samrai\samrai-v0.2.0-rc.8\.env" "C:\samrai\samrai-v0.2.0-rc.9\.env" -Force
